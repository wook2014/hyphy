...
3. **Modulo and Typo Bugs**: When performing modulo operations (e.g., `filteringOption % 2`), HBL converts the string to a number first (resulting in `0` for these non-numeric strings), which meant `filteringOption % 2` always evaluated to `0` and gap-filtering never executed. In addition, there was a typo on line 157 (`filterinOption == 4` instead of `filteringOption == 4`), which meant the `"Disallow stops"` option failed to correctly filter out sequences containing stop codons.
